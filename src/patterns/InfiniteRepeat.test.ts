...
describe("InfiniteRepeat", () => {
  ...
  test("Clone With Custom Overrides", () => {
    const numbers = new InfiniteRepeat("numbers", new Regex("number", "\\d"), {
      min: 3,
      divider: new Literal("divider", "divider"),
      trimDivider: true,
    });
    const clone = numbers.clone();
    const expected = new InfiniteRepeat("numbers", new Regex("number", "\\d"), {
      min: 3,
      divider: new Literal("divider", "divider"),
      trimDivider: true,
    });
  ...
  test("Trim Trailing Complex Delimiter Pattern", () => {
    const numbers = new InfiniteRepeat("numbers", new Regex("number", "\\d"), {
      divider: new Sequence("comma", [new Literal(",", ","), new Literal("space", " ")]),
      trimDivider: true,
    });
