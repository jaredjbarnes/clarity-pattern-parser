...
describe("Expression Pattern", () => {
  test("Single Expression", () => {
    ...
    // "*" is declared before "+", so it binds tighter and nests deeper.
    const precedence = expression.exec("a + b * c").ast;
    expect(precedence?.name).toBe("add-sub-expression");
    expect(precedence?.children[2].name).toBe("mult-div-expression");
    expect(precedence?.children[2].value).toBe("b * c");

    const malformed = expression.exec("a + b * ?");
    expect(malformed.ast).toBeNull();
    expect(malformed.cursor.hasError).toBe(true);
  });

  test("Tail", () => {
    const expression = createTailExpression();
    const inputs = ["a", "a.b", "a.b.c", "a.b.c()()()"];
    ...
    // Postfix chains are left associative, so "a.b.c" nests as "(a.b).c".
    const ast = expression.exec("a.b.c").ast;
    expect(ast?.name).toBe("refinement-expression");
    expect(ast?.children[0].name).toBe("refinement-expression");
    expect(ast?.children[0].value).toBe("a.b");
    expect(ast?.children[1].value).toBe(".c");
  });
  ...
  test("Options like", () => {
    ...
    // An expression of bare atoms behaves like Options: "a" fully matches, so
    // there is nothing left to suggest.
    expect(suggestion.isComplete).toBe(true);
    expect(suggestion.options).toEqual([]);
    expect(suggestion.errorAtIndex).toBeNull();
  });
  ...
  test("Suggest", () => {
    ...
    // After "a ? b " the ternary still needs its ":", and any infix operator
    // could continue the second operand instead.
    expect(suggestion.isComplete).toBe(false);
    expect(suggestion.options.map(o => o.text).sort()).toEqual(
      ["&& ", "* ", "+ ", "- ", "/ ", ":", "?", "|| "].sort()
    );
  ...
  test("Clone With New Name", () => {
    ...
    expect(clone.name).toBe("new-expression");
    expect(clone.type).toBe("expression");

    const result = clone.exec("a || b && c * d");
    expect(result.cursor.hasError).toBe(false);
    expect(result.ast?.value).toBe("a || b && c * d");

    // The clone keeps the original's precedence: "*" binds tightest, while
    // "||" and "&&" share a level and so nest to the left.
    expect(result.ast?.name).toBe("bool-expression");
    expect(result.ast?.children[0].value).toBe("a || b");
    expect(result.ast?.children[2].name).toBe("mult-div-expression");
    expect(result.ast?.children[2].value).toBe("c * d");
  });
  ...
  test("getTokensAfter for atom child with parent includes parent next tokens", () => {
    ...
    const wrapper = new Sequence("wrapper", [expression, new Literal("end", ";")]);
