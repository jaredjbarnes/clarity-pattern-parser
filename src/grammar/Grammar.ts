...
function defaultImportResolver(
  _path: string,
...
export class Grammar {
  ...
  private _buildPatternRecord() {
    ...
    this._parseContext.patternsByName.forEach((p, name) => {
      patterns[name] = new Context(
        name,
        p,
        allPatterns.filter(o => o !== p)
      );
    });
  ...
  private _buildPatterns(ast: Node) {
    ...
    for (let i = 0; i < allStatements.length; i++) {
      ...
      if (statementNode.name === "patternAssignment") {
        ...
        const patternExprNode = statementNode.children.find(
          n => n.name !== "patternName" && !skipNodes[n.name] && n.name !== "assign"
        );
  ...
  private _buildPattern(name: string, node: Node): Pattern {
    switch (node.name) {
      ...
      case "blockDelimiter":
        throw new Error(
          `Block delimiter [${this._extractBlockDelimiterValue(node)}] must be used at both the start and end of a sequence. Example: ["{"] content ["}"]`
        );
    }
  ...
  private _buildBlock(name: string, node: Node) {
    ...
    const openLiteral = new Literal(
      "open",
      this._extractBlockDelimiterValue(delimiters[0])
    );
    const closeLiteral = new Literal(
      "close",
      this._extractBlockDelimiterValue(delimiters[1])
    );

    // Find the content node — everything that isn't a delimiter or whitespace
    const contentChildren = node.children.filter(
      n => n.name !== "blockDelimiter" && !skipNodes[n.name]
    );
    ...
    const contentPattern = this._buildPattern(
      `anonymous-pattern-${anonymousIndexId++}`,
      contentChildren[0]
    );
  ...
  private _buildRepeat(name: string, node: Node) {
    ...
    const patternNode = node.children.find(
      n =>
        !skipNodes[n.name] &&
        n.name !== "(" &&
        n.name !== ")" &&
    ...
    if (delimiterNode != null) {
      const delimPatternNode = delimiterNode.children.find(
        n =>
          !skipNodes[n.name] &&
          n.name !== "," &&
          n.name !== "optionalTrim" &&
      ...
      if (delimPatternNode != null) {
        options.divider = this._buildPattern(
          `anonymous-pattern-${anonymousIndexId++}`,
          delimPatternNode
        );
      }
    ...
    const repeatOptionsNode = node.find(
      n => n.name === "oneOrMore" || n.name === "zeroOrMore" || n.name === "repeatBounds"
    );
  ...
  private _buildPatternGroup(_name: string, node: Node) {
    const innerNode = node.children.find(
      n => !skipNodes[n.name] && n.name !== "(" && n.name !== ")"
    );
  ...
  private _buildNot(_name: string, node: Node) {
    ...
    const inner = this._buildPattern(
      `anonymous-pattern-${anonymousIndexId++}`,
      innerNode
    );
  ...
  private _buildRightAssociation(node: Node) {
    ...
    return new RightAssociated(
      this._buildPattern(`anonymous-pattern-${anonymousIndexId++}`, innerNode)
    );
  }

  private _buildTakeUntil(name: string, node: Node) {
    // The last meaningful child is the until pattern
    const patternChildren = node.children.filter(
      n =>
        !skipNodes[n.name] &&
        n.name !== "anyChar" &&
        n.name !== "upTo" &&
    ...
    const untilPattern = this._buildPattern(
      `anonymous-pattern-${anonymousIndexId++}`,
      untilPatternNode
    );
  ...
  private _applyDecorators(
    ...
    for (const d of decoratorNodes) {
      ...
      if (actualDecorator.name === "nameDecorationStatement") {
        ...
        const jsonValueNode = actualDecorator.find(
          n =>
            n.name === "jsonArray" ||
            n.name === "jsonObject" ||
            n.name === "jsonString" ||
  ...
  private _processImportNames(
    ...
    for (const child of patternNamesNode.children) {
      if (child.name === "importAlias") {
        ...
        if (pattern == null) {
          throw new Error(
            `Couldn't find pattern with name: ${importName}, from import: ${resource}.`
          );
        }
