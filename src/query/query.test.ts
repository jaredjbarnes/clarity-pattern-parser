...
describe("Query", () => {
  ...
  test("Parent", () => {
    ...
    expect(result.map(n => n.value).join(",")).toBe(
  ...
  test("Not", () => {
    ...
    expect(result.map(n => n.value).join(",")).toBe(
      "5678,9101112,13141516,17181920,21222324"
    );
  });
  ...
  test("First Before", () => {
    ...
    $(root, "child")
      .first()
      .before(() => Node.createValueNode("node", "adopted-child", "0"));
  ...
  test("Last After", () => {
    ...
    $(root, "child")
      .last()
      .after(() => Node.createValueNode("node", "adopted-child", "25"));
  ...
  test("Append", () => {
    ...
    expect(parents.map(n => n.toString()).join(",")).toBe(
      "12345,56789,910111213,1314151617,1718192021,2122232425"
    );
  });
  ...
  test("Prepend", () => {
    ...
    expect(haveFiveChildren).toBeTruthy();
    expect(parents.map(n => n.toString()).join(",")).toBe(
      "01234,45678,89101112,1213141516,1617181920,2021222324"
    );
  });
  ...
  test("Compact", () => {
    ...
    const length = $(root, "parent").compact().find("child").length();
  ...
  test("Slice", () => {
    ...
    const array = $(root, "parent").slice(0, 1).toArray();
