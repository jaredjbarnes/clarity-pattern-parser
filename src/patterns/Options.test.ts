...
describe("Options", () => {
  ...
  test("Get Next Tokens", () => {
    const sequence = new Sequence("sequence", [
      new Options("a-or-b", [new Literal("a", "A"), new Literal("b", "B")]),
      new Literal("c", "C"),
    ]);
  ...
  test("Get Next Tokens With Null Parent", () => {
    const or = new Options("a-or-b", [new Literal("a", "A"), new Literal("b", "B")]);
  ...
  test("Get Patterns", () => {
    ...
    const expected = [aOrB.find(p => p.name === "a"), aOrB.find(p => p.name === "b")];
  ...
  test("Deeper Cyclical Error Recorvery", () => {
    ...
    const expression = new Options("expression", [john, johns], true);
