...
describe("grammar", () => {
  ...
  test("import with params", () => {
    const { ast } = grammar.exec(
      'import {pattern} from "resource" with params {param = "value"}'
    );
    ...
    expect(ast?.find((p: Node) => p.name === "withParamsExpr")?.value).toBe(
      'with params {param = "value"}'
    );
    expect(
      ast
        ?.find((p: Node) => p.name === "statements")
        ?.find((p: Node) => p.name === 'param = "value"')
    ).toBeDefined();
  });
  ...
  test("use params", () => {
    ...
    expect(ast?.find((p: Node) => p.name === "useParamsStatement")?.value).toBe(
      "use params { param }"
    );
  ...
  test("literal", () => {
    ...
    expect(ast?.find((p: Node) => p.name === "patternAssignment")?.value).toBe(
      'literal = "Hello, world!"'
    );
    expect(ast?.find((p: Node) => p.name === "patternName")).toBeDefined();
  ...
  test("sequence", () => {
    ...
    expect(ast?.find((p: Node) => p.name === "sequenceExpr")?.value).toBe(
      '"Hello" + "world"'
    );
  });
  ...
  test("options", () => {
    ...
    expect(ast?.find((p: Node) => p.name === "optionsExpr")?.value).toBe(
      '"Hello" | "world"'
    );
  });
  ...
  test("greedy options", () => {
    ...
    expect(ast?.find((p: Node) => p.name === "greedyOptionsExpr")?.value).toBe(
      '"Hello" <|> "world"'
    );
  });
  ...
  test("repeat with delimiter", () => {
    ...
    expect(ast?.find((p: Node) => p.name === "patternName")?.value).toBe("repeat");
    expect(ast?.find((p: Node) => p.name === "repeatExpr")).toBeDefined();
    expect(ast?.find((p: Node) => p.name === "repeatExpr")?.value).toBe(
      '(Hello, "world")+'
    );
  });
  ...
  test("take until", () => {
    ...
    expect(ast?.find((p: Node) => p.name === "takeUntilExpr")?.value).toBe(
      '?->| "</script"'
    );
