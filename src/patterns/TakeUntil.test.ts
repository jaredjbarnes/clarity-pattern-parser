...
describe("TakeUntil", () => {
  ...
  test("test() returns true when content exists before terminator", () => {
    const takeUntil = new TakeUntil("body", new Literal("end", "</end"));
  ...
  test("id and type accessors", () => {
    const takeUntil = new TakeUntil("content", new Literal("end", ";"));
  ...
  test("clone creates independent copy with same name", () => {
    const takeUntil = new TakeUntil("script-content", new Literal("close", "</script"));
